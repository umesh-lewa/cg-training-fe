...
customElements.define('items-comp', Items);
customElements.define('details-comp', Details);
customElements.define('pokemon-comp', Pokemon);

/*
async function getTopTenPokemon(event){
  event.preventDefault();

  let searchTerm = document.getElementById("search-bar").value;
  let url = "https://pokeapi.co/api/v2/pokemon?limit=10&offset=20";
  ...
  for(let i = 0 ; i < 10 ; i++){
    // add each result obj to the list component
...
async function searchPokemon(event){
  ...
  let pokeminHeight = pokemon.height;
  let pokemonOrder = pokemon.order;
  let pokemonAbilities = pokemon.abilities;

  for(let i = 0 ; i < pokemonAbilities.length; i++){
    // add each result obj to the list component
    let ability = pokemonAbilities[i].ability;
